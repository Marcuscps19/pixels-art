...
function clearPixelElement(event) {
  event.target.style.setProperty(bgColor, 'white');
}
...
function createPixelsListeners() {
  for (let index = 0; index < pixels.length; index += 1) {
    ...
    pixels[index].addEventListener('dblclick', clearPixelElement);
  }
}
...
function clickedPaletteElement() {
  ...
  selectedPalette = window.getComputedStyle(this).getPropertyValue(bgColor);
  removeSelected(this);
}
